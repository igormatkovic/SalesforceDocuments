          ...
          <template v-for="field in fields">
            <template v-if="field.visible">
              <template v-if="isSpecialField(field.name)">
                ...
                <td valign="middle" v-if="extractName(field.name) === '__component'" :class="['vuetable-component', field.dataClass]">
...
export default {
  ...
  methods: {
    ...
    getAllQueryParams: function() {
      ...
      params[this.queryParams.showDeleted] = this.showDeleted
    ...
    toggleCheckbox: function(dataItem, fieldName, event) {
      ...
      Vuemit.fire('vuetable:checkbox-toggled', {
        isChecked: isChecked,
        totalSelected:  this.selectedTo.length,
        data: dataItem
      })
    },
